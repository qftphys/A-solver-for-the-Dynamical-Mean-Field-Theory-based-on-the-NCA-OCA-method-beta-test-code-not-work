...
contains
  ...
  subroutine nca_get_observables()
    ...
    do iorb=1,Norb
       Matrix = 0d0
       !>DEBUG
       !this a fix: use index Nspin rather than 2, because the n_dw are not evaluated
       !correctly. I can not find  the reason yet.
    ...
    !
    write(LOGfile,"(A,10f18.12)")       "dens UP=",(nca_dens_up(iorb),iorb=1,Norb)
    write(LOGfile,"(A,10f18.12)")       "dens DW=",(nca_dens_dw(iorb),iorb=1,Norb)
